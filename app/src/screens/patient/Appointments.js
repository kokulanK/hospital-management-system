...
import { View, Text, FlatList, TouchableOpacity, Modal, Alert, StyleSheet, ActivityIndicator, Platform } from 'react-native';
...
export default function PatientAppointments() {
  ...
  const handleBook = async () => {
    if (!selectedDoctor || !selectedSlot) {
      Alert.alert('Error', 'Please select doctor and slot');
      return;
    }
    setSubmitting(true);
    try {
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.doctorOption,
                      selectedDoctor?._id === item._id && styles.doctorOptionActive,
                    ]}
                  >
                    <Text style={[styles.doctorText, selectedDoctor?._id === item._id && styles.doctorTextActive]}>
                      Dr. {item.name}
                    </Text>
                  </TouchableOpacity>
                )}
              ...
              <View style={styles.slotsRow}>
                {slots.map((slot, idx) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.slotBtn,
                      selectedSlot?.start === slot.start && styles.slotBtnActive,
                    ]}
                  >
