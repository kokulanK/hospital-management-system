...
export default function DashboardLayout({ children, activePage }) {
  ...
  const links = [
    { to: '/dashboard/receptionist', icon: FaHome, label: 'Home', key: 'home' },
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      <style>{`
        ...
        .recep-sidebar { font-family: 'DM Sans', sans-serif; }
        .recep-main { font-family: 'DM Sans', sans-serif; }
        .sidebar-link {
      ...
      {/* Sidebar */}
      <aside className="recep-sidebar fixed top-0 left-0 h-screen w-20 bg-white flex flex-col items-center justify-between py-5"
      ...
      {/* Main */}
      <main className="recep-main flex-1 ml-20 p-8 min-h-screen" style={{ background: '#f8faff' }}>
        {children}
      </main>
    </div>
  );
