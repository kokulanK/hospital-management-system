// frontend/src/pages/cleaning/DashboardLayout.jsx
...
export default function DashboardLayout({ children, activePage }) {
  ...
  const links = [
    { to: '/dashboard/cleaning', icon: FaHome, label: 'Home', key: 'home' },
    { to: '/dashboard/cleaning/tasks', icon: FaClipboardList, label: 'My Tasks', key: 'tasks' },
    { to: '/dashboard/cleaning/supplies', icon: FaBox, label: 'Supply Requests', key: 'supplies' },
    { to: '/dashboard/cleaning/profile', icon: FaUser, label: 'Profile', key: 'profile' },
  ];
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      <style>{`
        ...
        .clean-sidebar { font-family: 'DM Sans', sans-serif; }
        .sidebar-link {
      ...
      <aside
        className="clean-sidebar fixed top-0 left-0 h-screen w-20 bg-white flex flex-col items-center justify-between py-5"
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1, justifyContent: 'center' }}>
          {links.map(({ to, icon: Icon, label, key }) => (
            <a key={to} href={to} className={`sidebar-link ${activePage === key ? 'active' : ''}`}>
              <Icon size={20} />
