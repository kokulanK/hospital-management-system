...
export default function AIDoctorAssistant() {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', content: 'Hello, Doctor! I am your AI Clinical Assistant. Ask me about drug interactions, ICD-10 codes, or standard treatment protocols.' }
  ]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      const systemPrompt = "You are an expert AI Clinical Assistant designed to help a physician. Answer questions strictly related to medical diagnoses, ICD-10 codes, drug interactions, contraindications, and standard treatment protocols. Provide highly professional, concise, and accurate medical information. If a question is unrelated to medicine or hospital workflows, politely decline to answer.";
      ...
      const errorMsg = "Sorry, I'm having trouble connecting to my clinical knowledge base right now.";
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 p-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-full shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 z-40 ${isOpen ? 'scale-0' : 'scale-100'}`}
      ...
      <div className={`fixed bottom-6 right-6 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl z-50 flex flex-col transition-all duration-300 origin-bottom-right ${isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`} style={{ height: '500px', maxHeight: '80vh' }}>
        ...
        {/* Header */}
        <div className="px-5 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex justify-between items-center rounded-t-2xl">
