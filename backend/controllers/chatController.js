...
const formatContextForAI = (patientData) => {
  ...
  if (patientData.appointments.length) {
    patientData.appointments.forEach(apt => {
      context += `- ${new Date(apt.date).toLocaleString()}: Dr. ${apt.doctor.name}, status: ${apt.status}\n`;
    });
  } else {
  ...
  if (patientData.feedbacks.length) {
    patientData.feedbacks.forEach(fb => {
      context += `- ${new Date(fb.createdAt).toLocaleDateString()}: rating ${fb.rating}, comment: ${fb.comment || 'none'}\n`;
    });
  } else {
...
const getFallbackResponse = (message, patientData) => {
  ...
  if (msg.includes('appointment') || msg.includes('book') || msg.includes('schedule')) {
    ...
    if (upcoming.length) {
      ...
      return `Your next appointment is with Dr. ${next.doctor.name} on ${new Date(next.date).toLocaleString()}.`;
    } else {
      const last = appointments[0];
      return `Your last appointment was with Dr. ${last.doctor.name} on ${new Date(last.date).toLocaleString()}. You have no upcoming appointments.`;
    }
  }
